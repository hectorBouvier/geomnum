...
def Subdivide( M0, u_closed, v_closed ) :
    # dim is equal to 3
    dim, m, n = M0.shape
    M2=np.empty([dim,m,2*n-2*(1-v_closed)])
    for i in range(m):
        M2[:,i,:]=Chaikin(M0[:,i,:],v_closed)
    print(M2)
    M1=np.empty([dim,2*m-2*(1-u_closed),2*n-2*(1-v_closed)])
    for i in range(2*n-2*(1-v_closed)):
        M1[:,:,i]=Chaikin(M2[:,:,i],u_closed)
    print(M1)
    return M1

def Chaikin(X0,closed):
    # number of points
    n = X0.shape[1]
    # upsample
    X1 = np.zeros([3,2 * n-2*(1-closed)])
    if closed :
        for i in range(n):
            # modulo n pour eviter out of bounds et fermer la courbe
            X1[:,2 * i] = (3.0 / 4.0) * X0[:,i] + (1.0 / 4.0) * X0[:,(i + 1) % n]
            X1[:,(2 * i + 1)] = (1.0 / 4.0) * X0[:,i] + (3.0 / 4.0) * X0[:,(i + 1) % n]
    else:
        for i in range(n-1):
            X1[:,2 * i] = (3.0 / 4.0) * X0[:,i] + (1.0 / 4.0) * X0[:,(i + 1)]
            X1[:,(2 * i + 1)] = (1.0 / 4.0) * X0[:,i] + (3.0 / 4.0) * X0[:,(i + 1)]
