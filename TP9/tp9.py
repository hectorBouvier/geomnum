...
def InsertMidpoints(V,F) :
    ...
    for i in range(f) :
        ...
        for j in range(3) :
            ...
            if E[i,2*j+1] == -1 :
                faces, eindex, oppos =FindEdges(F,E[i,2*j],E[i,(2*j+2)%6])  #recherche de l'autre face concernee
                E[faces[0],eindex[0]*2+1]=m #premiere face
                E[faces[1],eindex[1]*2+1]=m #deuxieme face
                M[m,:]=(V[E[i,2*j]]+V[E[i,(2*j+2)%6]])/2    #calcul du point (milieu)
                m+=1    #on passe au point suivant
    return M, E
...
def Beta(N,Warren) :
    if (Warren):
        return 3/16 if N==3 else 3/8/N
    else :
        return 1.0/N*(5.0/8-(3.0/8+1.0/4*math.cos(2*math.pi/N))**2)
...
def RecomputePositions(V,F,Warren) :
    ...
    # update old vertices
    for i in range(v) :
        neighbour= GetAdjacentVertices(F,i)
        n=neighbour.shape[0]+1
        b=Beta(n,Warren)
        newV[i]=(1-n*b)*V[i]+sum([b*V[j] for j in neighbour])
