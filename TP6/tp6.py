...
def DeCasteljau(M,u,v) :    
    ...
    b=np.zeros(n)
    for j in range(n):
        b[j]=DeCasteljauR(M[j,:],m-1,0,u)
    return DeCasteljauR(b,n-1,0,v)

def DeCasteljauR( BezierPts, k, i, t ) :
    if k==0:
        return BezierPts[i]
    else: 
        return t*DeCasteljauR(BezierPts,k-1,i+1,t)+(1-t)*DeCasteljauR(BezierPts,k-1,i,t)
